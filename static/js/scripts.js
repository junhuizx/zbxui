...
(function() {
   ...
   function searchform_reposition() {
    ...
    function get_latest_data(){
        var url = '/zabbix/reload/';
        $.get(url, function(data){
            if (data['flag']=='success'){
                $('div.zabbix_issue').empty();

                $('div.zabbix_issue').append(data['issues']);
            };
        });
    }
    //Dynamic reload latest data page
    $(function(){
        var current_url = window.location.href;
        if (current_url.indexOf('/zabbix/')!=-1){
            setInterval(get_latest_data, 60000);
        }
    })
    // tool tips
